...
export default function SignUp({ updateIsLog }) {
  ...
  function handlePassword(event) {
    ...
    if (password === "") {
      ...
      setMassageWarning({
        ...massageWarning,
        password: `يرجى ادخال كلمة المرور التي تتكون من 8 أحرف على الأقل وتحتوي على حرف كبير واحد ، ورقم واحد ، ورمز خاص واحد `,
      });
    } else {
  ...
  function handleConfirmPassword(event) {
    ...
    if (password === "") {
      ...
      setMassageWarning({
        ...massageWarning,
        confirmPassword: "كملة المرور غير متطابقة",
      });
    } else {
  ...
  function handleSubmit(event) {
    ...
    if (selectedUserType === "donor") {
      ...
      setMassageWarning({
        ...massageWarning,
        submit: "الرجاء ادخال رقم وطني صالح",
      });
      return;
    }
    ...
    if (
      ...
      setMassageWarning({
        ...massageWarning,
        submit: "الرجاء ادخال جميع الحقول المطلوبة",
      });
    }
  }
  ...
  async function sendDataToServer(user) {
    try {
      const res = await axios.post(
        `http://localhost:8000/${
          selectedUserType === "donor" ? "donor" : "charity"
        }`,
        user
      );
      ...
      if (selectedUserType === "charity") {
        setMassageWarning({
          ...massageWarning,
          submit:
            "تمت العملية بنجاح ،يجب عليك انتظار موافقة المسؤول الان، سوف تصلك رسالة بريد الكترونية عندما تتم الموافقة او الرفض",
        });
        return;
      }
        ...
        <div class="lg:w-1/2 xl:w-5/12 p-6 sm:p-12 sm:w-10/12">
          ...
          <div class=" flex flex-col items-center ">
            <div class="w-full flex-1 mt-8">
              <div class="flex flex-col items-center">
                <SignGoogle
                  massage={"التسجيل بواسطة جوجل"}
                  selectedUserType={selectedUserType}
                  updateIsLog={updateIsLog}
                />

                <Facebook
                  massage={"التسجيل بواسطة فيسبوك"}
                  selectedUserType={selectedUserType}
                  updateIsLog={updateIsLog}
                />
              </div>
              ...
              <form onSubmit={(event) => handleSubmit(event)}>
                <div class="mx-auto max-w-xs">
                  ...
                  <div class="mb-3">
                    <label
                      for="name"
                      className={`block mb-2 text-sm font-medium text-${inputTheme.username}-700 dark:text-${inputTheme.username}-500 `}
                      style={{ direction: "rtl" }}
                    >
                      {selectedUserType === "charity" ? "اسم الجمعية" : "الاسم"}
                    </label>
                    <input
                    ...
                    <p
                      ...
                      style={{ direction: "rtl" }}
                    >
                      <span class="font-medium">{massageWarning.name}</span>
                    </p>
                  </div>
                  <div class="mb-3">
                    <label
                      for="phone"
                      className={`block mb-2 text-sm font-medium text-${inputTheme.phone}-700 dark:text-${inputTheme.phone}-500 `}
                      style={{ direction: "rtl" }}
                    >
                      رقم الهاتف
                    </label>
                    ...
                    <p
                      ...
                      style={{ direction: "rtl" }}
                    >
                      <span class="font-medium">{massageWarning.phone}</span>
                    </p>
                  </div>
                  <div class="mb-3">
                    <label
                      for="email"
                      className={`block mb-2 text-sm font-medium text-${inputTheme.email}-700 dark:text-${inputTheme.email}-500 `}
                      style={{ direction: "rtl" }}
                    >
                      البريد الالكتروني
                    </label>
                    ...
                    <p
                      ...
                      style={{ direction: "rtl" }}
                    >
                      <span class="font-medium">{massageWarning.email}</span>
                    </p>
                  </div>
                  <div class="mb-3 password">
                    <label
                      for="password"
                      className={`text-${inputTheme.password}-700 dark:text-${inputTheme.password}-500 block mb-2 text-sm font-medium`}
                      style={{ direction: "rtl" }}
                    >
                      كلمة المرور
                    </label>
                    ...
                    <p
                      ...
                      style={{ direction: "rtl" }}
                    >
                      <span class="font-medium">{massageWarning.password}</span>
                    </p>
                  </div>
                  <div className="password">
                    <label
                      for="confirmPassword"
                      className={`text-${inputTheme.confirmPassword}-700 dark:text-${inputTheme.confirmPassword}-500 block mb-2 text-sm font-medium`}
                      style={{ direction: "rtl" }}
                    >
                      تأكيد كلمة المرور
                    </label>
                  ...
                  >
                    ...
                    <span class="ml-3 ">التسجيل الان</span>
                  </button>
                  <p
                    className={`mt-2 text-sm text-${themeValue.warning}-600 dark:text-${themeValue.warning}-500`}
                    style={{ direction: "rtl" }}
                  >
                    <span class="font-medium">{massageWarning.submit}</span>
                  </p>
                  <p
                    className={`mt-2 text-sm text-${themeValue.normal}-600 dark:text-${themeValue.normal}-500`}
                    style={{ direction: "rtl" }}
                  >
                    لديك حساب بالفعل!{" "}
                    <Link
                      to="/signIn"
                      className={`font-bold text-${themeValue.normal}-300 transition hover:text-${themeValue.normal}-500/75`}
                    >
                      تسجيل الدخول
                    </Link>
                  </p>
